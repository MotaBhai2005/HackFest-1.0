...
function CountdownTimer() {
	...
	return (
		<div className="flex flex-col items-end gap-1">
			<span className="font-display font-bold text-lg md:text-[22px] text-hack-red tracking-tight leading-none">T minus</span>
			<motion.div
...
export default function Hero() {
	...
	return (
		<section ref={heroRef} id="hero" className="bg-hack-yellow min-h-screen overflow-hidden relative">
			{/* ===== HF Logo — top left ===== */}
			<motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1, duration: 0.15 }} className="absolute top-6 md:top-8 left-6 md:left-12 z-20">
				<img src="images/hf-logo.svg" alt="HF Logo" className="w-16 h-[29px] md:w-24 md:h-[43px]" onError={(e) => ((e.target as HTMLImageElement).style.display = "none")} />
			</motion.div>
			...
			>
              ...
              ${link.active ? "text-hack-red font-semibold text-base md:text-xl" : "text-[#c9a102] hover:text-hack-red"}`}
					>
						{link.label}
					</motion.a>
				))}
			...
			<div className="relative w-full h-screen flex items-center justify-center">
				...
				>
					...
					<div className="relative overflow-hidden">
						...
						<img src="images/hero-hacker.png" alt="" className="glitch-clone red z-[2]" aria-hidden="true" />

						{/* Cyan channel clone — offset in opposite direction */}
						<img src="images/hero-hacker.png" alt="" className="glitch-clone cyan z-[3]" aria-hidden="true" />
				...
				<motion.div
					...
					initial={{ opacity: 0, y: 40 }}
					animate={glitchCycle ? { opacity: 1, y: 0, x: [0, -3, 5, -2, 0], skewX: [0, 2, -3, 1, 0] } : { opacity: 1, y: 0, x: 0, skewX: 0 }}
					transition={glitchCycle ? { duration: 0.3, ease: "linear" } : { delay: 0.25, duration: 0.2 }}
				>
					<img src="images/hackfest-text-outline.svg" alt="" className="w-full h-auto" onError={(e) => ((e.target as HTMLImageElement).style.display = "none")} />
				</motion.div>
			</div>
			...
			<div className="absolute bottom-4 md:bottom-8 left-6 md:left-12 flex items-center gap-2 z-20">
				<HudDataChip label="STATUS" value="AWAITING_DEPLOY" status="error" tooltip="SYSTEM NOT YET LIVE" />
			</div>
		</section>
	);
}
